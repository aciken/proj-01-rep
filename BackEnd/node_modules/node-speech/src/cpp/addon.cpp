...
* @function SpeakSsml
*
* Speak a given string in Ssmlformat
*
* @return   {napi_string}
*/
napi_value SpeakSsml(napi_env env, napi_callback_info info)
{
    napi_status status;
    ...
    ManagedRecognizer::_speakSslm(text);
...
napi_value InitModule(napi_env env, napi_value exports) {
    ...
    napi_property_descriptor speak_ssml = DECLARE_NAPI_METHOD("speak_ssml", SpeakSsml);
    status = napi_define_properties(env, exports, 1, &speak_ssml);
    assert(status == napi_ok);
